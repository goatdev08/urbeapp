...
- **OTA a testers (EAS Update): `cd mobile && pnpm ota "<mensaje>" [android|ios|all]`** (default `all`). Publica a los canales `production`(iOS) y `preview`(Android), runtime `1.0.1`. Correr **desde `main` ya mergeado** (el OTA sube el working dir; el script avisa si no estás en main). ⚠️ **`eas update` directo TRUENA bajo pnpm** (bundler roto) → el script separa `expo export` + `eas update --skip-bundler` y usa `npx -y eas-cli@latest` (la global está rota). Solo JS/UI viaja por OTA; **módulo nativo nuevo = subir `version` + recompilar build**, no OTA. Ver `mobile/scripts/ota.sh` y [[estrategia-releases]].

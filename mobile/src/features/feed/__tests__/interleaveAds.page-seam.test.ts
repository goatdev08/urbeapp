...
// ── #285.4 — memoria del min_gap ENTRE composiciones ───────────────────────
// `last_shown_at` es local a cada llamada: `since_last_ad` (256) evita dos
// anuncios seguidos en la costura, pero NO recuerda cuántos ítems lleva cada
// anuncio sin aparecer. Con un solo anuncio activo, every_n=8 y páginas de 8
// (producción hoy), cada composición cierra con él (#247) y la siguiente lo
// vuelve a servir en cuanto está "due": 9 ítems de distancia con
// min_gap_between_repeats=16. Con la vuelta infinita (#285) eso es el mismo
// anuncio cada 9 ítems para siempre. Contrato nuevo: `ad_distances`
// (Map ad.id → ítems emitidos desde su última aparición, al cerrar la llamada
// anterior) entra por opts y sale actualizado en el resultado; sin la opción
// el comportamiento es el de siempre.
describe('interleave_ads_with_state — ad_distances: min_gap con memoria entre composiciones (#285.4)', () => {
  const OPTS = { every_n: 8, max_per_session: 10, min_gap_between_repeats: 16, already_shown_count: 0 } as const;

  it('(EC-285-1) sin_ad_distances_el_anuncio_reaparece_a_9_items_y_con_ad_distances_heredado_espera_el_min_gap', () => {
    const ad = make_ad('unico');
    const page_1 = interleave_ads_with_state(make_properties(8, 'p1'), [ad], { ...OPTS, skip_first_position: true });
    expect(page_1.items.filter((it) => it.kind === 'ad')).toHaveLength(1); // cierra con el anuncio (#247)

    // Sin memoria (comportamiento actual): la página 2 lo vuelve a servir.
    const page_2_sin = interleave_ads_with_state(make_properties(8, 'p2'), [ad], {
      ...OPTS, skip_first_position: false, already_shown_count: 1, since_last_ad: page_1.since_last_ad,
    });
    expect(page_2_sin.items.filter((it) => it.kind === 'ad')).toHaveLength(1);

    // Con memoria: al cerrar la página 1 el anuncio lleva 0 ítems desde su
    // última aparición; la página 2 (8 propiedades) no alcanza el min_gap=16.
    expect(page_1.ad_distances.get(ad.id)).toBe(0);
    const page_2_con = interleave_ads_with_state(make_properties(8, 'p2'), [ad], {
      ...OPTS, skip_first_position: false, already_shown_count: 1, since_last_ad: page_1.since_last_ad,
      ad_distances: page_1.ad_distances,
    });
    expect(page_2_con.items.filter((it) => it.kind === 'ad')).toHaveLength(0);
    expect(page_2_con.ad_distances.get(ad.id)).toBe(8); // 8 propiedades emitidas desde entonces

    // Página 3 con la memoria de la 2: 8 + 8 = 16 ≥ min_gap → vuelve a servirse (una vez).
    const page_3 = interleave_ads_with_state(make_properties(8, 'p3'), [ad], {
      ...OPTS, skip_first_position: false, already_shown_count: 1, since_last_ad: page_2_con.since_last_ad,
      ad_distances: page_2_con.ad_distances,
    });
    expect(page_3.items.filter((it) => it.kind === 'ad')).toHaveLength(1);
  });

  it('(EC-285-2) ad_distances_no_muta_el_map_recibido_y_sin_la_opcion_los_items_son_identicos_a_hoy', () => {
    const ads = make_ads(2);
    const inherited = new Map<string, number>([[ads[0]!.id, 0]]);
    const snapshot = new Map(inherited);
    const with_opt = interleave_ads_with_state(make_properties(8), ads, { ...OPTS, skip_first_position: false, ad_distances: inherited });
    expect(inherited).toEqual(snapshot); // pureza: el caller decide cuándo adoptar el map nuevo
    expect(with_opt.ad_distances).not.toBe(inherited);

    const a = interleave_ads_with_state(make_properties(8), ads, { ...OPTS, skip_first_position: false });
    const b = interleave_ads(make_properties(8), ads, { ...OPTS, skip_first_position: false });
    expect(a.items).toEqual(b);
  });
});

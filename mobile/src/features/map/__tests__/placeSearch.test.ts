...
function make_mock_supabase(result: { data: RpcRow[] | null; error: { message: string } | null }) {
  // Candado #233.3: `rpc` sensible al binding (make_binding_sensitive_supabase_mock)
  // — el mutante `const { rpc } = client; rpc(...)` (#205) muere si alguien
  // lo reintroduce en placeSearch.ts. Spread conserva la forma `{rpc, _mock_rpc}`
  // que ya consumían las aserciones de este archivo (`mock._mock_rpc`).

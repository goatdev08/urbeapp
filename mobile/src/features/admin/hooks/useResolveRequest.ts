/**
 * useResolveRequest — resolver (aprobar/rechazar con motivo) las dos colas
 * mutables de /admin/requests (módulo 041-M4, subtarea 221.4): solicitudes
 * de agente y solicitudes de cuenta comercial. Contrato completo en
 * __tests__/useResolveRequest.test.ts.
 *
 * 🔴 CONTRATOS PINNEADOS (backend en paralelo, tarea 221.2/221.1, otra
 * rama) — RPCs, no Edge Functions:
 ...
 *     mismos 3 códigos, mismo patrón (el rechazo exige motivo).
 * Mismo criterio de parseo de error que create_ad_campaign_atomic
 * (`error.message.includes(code)`).
 *
 * Dos hooks SIBLING (no una fábrica genérica por RPC name): misma FORMA de
 * estado (is_working_ref/error_ref/force_update) pero cada uno con su propio
 * nombre de RPC y forma de params — mismo criterio que useReportUser/
 * useReportProperty (ponytail: 2 call sites con nombres de parámetro
 * DISTINTOS; una indirección genérica no ahorra nada legible hoy).
 *

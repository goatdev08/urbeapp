-- Migración 20260815000005 — moderate_property_atomic: el snapshot de
-- revisión también aplica built_square_meters/half_bathrooms/currency (quick
-- fixes wizard paso 3, sesión 2026-08-15, sin tarea de Taskmaster).
--
-- Mismo shape de argumentos que 20260809000007 (sin cambios de firma) — solo
-- se extiende la lista CASE del bloque (1) para los 3 campos nuevos de
-- 20260815000003, mismo patrón "por presencia de clave" (#142) que el resto.
-- No-críticos (§15.5, igual que square_meters/bathrooms): un admin que
-- aprueba una revisión los aplica junto con el resto del snapshot.
--
-- Idempotente: create or replace function (misma firma, sin DROP necesario).
-- Rollback: supabase/migrations/rollbacks/20260815000005_moderate_property_atomic_wizard_fields.sql
-- Tests: supabase/tests/39_moderate_property_atomic_test.sql (asserts 9-11).
...
comment on function public.moderate_property_atomic(
  ...
  '#130 + quick fixes wizard paso 3 (2026-08-15): el snapshot de revisión '
  '(p_changed_fields, por presencia de clave) ahora también aplica '
  'built_square_meters/half_bathrooms/currency, igual que square_meters/'
  'bathrooms. Resto idéntico a 20260809000007.';

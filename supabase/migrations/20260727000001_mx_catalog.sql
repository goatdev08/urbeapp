-- Migración 0032 — Catálogo oficial MX: estados y municipios (INEGI)
-- Tarea #72.1 (Ola 1, Auth completo). Aditiva e idempotente.
--
-- Propósito: dejar de guardar estado/ciudad como texto libre. Hoy public.users tiene
--   city/state text sin validación, así que 'Guadalajara', 'guadalajara' y 'GDL' son
--   tres ciudades distintas para los filtros del feed, del mapa y de la búsqueda.
--   Este catálogo es la fuente única: registro (§5.1), publicación (§14.3) y filtros.
--
-- Fuente: web service OFICIAL de INEGI — https://gaia.inegi.org.mx/wscatgeo/
--   · mgee/     -> 32 entidades federativas
--   · mgem/<cve> -> municipios de una entidad
--   Descargado 2026-07-27: 32 estados y 2478 municipios. (2478, no el 2469 que se cita
--   a menudo: INEGI ya incluye los municipios creados recientemente.)
--
-- Claves: se conserva la clave INEGI literal como PK, no un serial. El 'cvegeo' de un
--   municipio es la concatenación entidad+municipio ('14039' = Jalisco 14 + Guadalajara 039),
--   así que la PK une con cualquier dataset de INEGI sin tabla de traducción. Por eso son
--   text con zero-padding y no int: '09' != 9 al unir contra un CSV de INEGI.
--
-- RLS: catálogo de lectura pública — mismo patrón que public.terms_versions (0008).
--   anon NECESITA select: el usuario elige estado/ciudad durante el registro, o sea antes
--   de autenticarse. Sin ese grant el formulario de registro sale vacío.
--   ⚠️ El helper es `private.is_admin()`, NO `public.is_admin()`: 0010 movió los helpers de
--   RLS al schema `private` y dropeó los de `public` (0010:22 y 0010:394). 0008 todavía
--   muestra el nombre viejo — copiarlo de ahí revienta con 42883 al aplicar la migración.

-- ── Tablas ──────────────────────────────────────────────────────────────────
create table if not exists public.mx_states (
  id         text primary key check (id ~ '^[0-9]{2}$'),
  name       text not null,
  abbr       text not null,
  created_at timestamptz not null default now()
);
comment on table public.mx_states is 'Catálogo INEGI de entidades federativas. id = cve_agee.';

create table if not exists public.mx_municipalities (
  id         text primary key check (id ~ '^[0-9]{5}$'),
  state_id   text not null references public.mx_states (id) on delete restrict,
  name       text not null,
  created_at timestamptz not null default now(),
  -- Invariante real del cvegeo INEGI: sus 2 primeros dígitos SON la clave de la entidad.
  -- Atrapa un seed mal generado que la FK sola dejaría pasar (state_id válido pero ajeno).
  constraint mx_muni_cvegeo_coherente check (left(id, 2) = state_id)
);
comment on table public.mx_municipalities is 'Catálogo INEGI de municipios/demarcaciones. id = cvegeo (entidad+municipio).';

create index if not exists mx_municipalities_state_name_idx
  on public.mx_municipalities (state_id, name);

-- ── RLS: lectura pública; escritura SOLO por migración/service_role ─────────
-- Sin política de escritura a propósito: RLS habilitada + cero políticas de INSERT/
-- UPDATE/DELETE = denegado por defecto. El catálogo es dato oficial de INEGI, su
-- ciclo de vida es "se re-siembra con una migración cuando INEGI publique cambios",
-- no "un admin lo edita desde la app" (eso no existe en los mockups). Una política
-- `for all to authenticated using (private.is_admin())` sería letra muerta: a
-- `authenticated` nunca se le otorga INSERT/UPDATE/DELETE aquí, así que jamás se
-- evaluaría. Menos superficie y menos código que no hace nada.
alter table public.mx_states          enable row level security;
alter table public.mx_municipalities  enable row level security;

drop policy if exists mx_states_select on public.mx_states;
create policy mx_states_select on public.mx_states for select to anon, authenticated
  using (true);

drop policy if exists mx_municipalities_select on public.mx_municipalities;
create policy mx_municipalities_select on public.mx_municipalities for select to anon, authenticated
  using (true);

-- Grants. service_role explícito: el ALTER DEFAULT PRIVILEGES de 0014 solo cubre tablas
-- creadas por el mismo rol que aplicó esa migración; no dependemos de eso.
grant select on public.mx_states, public.mx_municipalities to anon, authenticated;
grant select, insert, update, delete on public.mx_states, public.mx_municipalities to service_role;

-- 🔒 TRUNCATE no lo filtra la RLS y Supabase lo regala de fábrica: `pg_default_acl`
-- muestra que toda tabla nueva de `public` nace con el bit D (TRUNCATE) para anon y
-- authenticated. O sea: sin este revoke, un cliente anónimo podría vaciar el catálogo
-- pese a la RLS y a no tener ni un solo grant de escritura. Lo sellamos en la frontera
-- que creamos aquí; el barrido de las otras 21 tablas de public va en la tarea #92.
revoke truncate on public.mx_states, public.mx_municipalities from anon, authenticated;

-- ── Seed: 32 entidades ──────────────────────────────────────────────────────
-- on conflict do update: idempotente Y absorbe renombres oficiales de INEGI.
insert into public.mx_states (id, name, abbr) values
...
-- ── Seed: 2478 municipios ───────────────────────────────────────────────────
insert into public.mx_municipalities (id, state_id, name) values

-- Rollback: 20260907100001_crm_leads_page_filtros.sql (subtarea 271.1)
-- Restaura public.crm_leads_page a la firma de 5 parámetros, BYTE POR BYTE idéntica a
-- 20260906100003_crm_leads_page_funnel.sql (sin p_status/p_follow_up). DROP+CREATE en UNA
-- transacción — la misma razón que la migración de ida: agregar/quitar parámetros no admite
-- `create or replace` porque cambia la identidad de la función en pg_proc.

begin;

drop function if exists public.crm_leads_page(uuid, text, jsonb, int, text, text[], boolean);

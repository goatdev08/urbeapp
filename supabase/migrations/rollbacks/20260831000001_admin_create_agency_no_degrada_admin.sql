-- Rollback: 20260831000001_admin_create_agency_no_degrada_admin.sql (tarea 225).
-- Restaura la definición ANTERIOR de public.admin_create_agency_atomic, la de
-- 20260816000004 (#168.7).
--
-- 🔴 QUÉ SIGNIFICA REVERTIR ESTO: se reintroduce el bug. El UPDATE del owner
-- vuelve a ir sin condicionar, así que asignar a un administrador como owner
-- de una organización nueva lo degrada a 'agent' y le quita el panel de
-- administrador. Es lo que un rollback DEBE hacer —volver al estado previo—,
-- pero conviene que quede escrito: no es un rollback inocuo.
--
-- Idempotente: create or replace. No toca firma, retorno ni grants (la
-- migración tampoco los cambió), así que no hace falta revocar nada.

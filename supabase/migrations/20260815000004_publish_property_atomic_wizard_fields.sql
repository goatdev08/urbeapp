-- Migración 20260815000004 — publish_property_atomic: acepta
-- p_built_square_meters, p_half_bathrooms, p_currency (quick fixes wizard
-- paso 3, sesión 2026-08-15, sin tarea de Taskmaster).
--
-- Extiende publish_property_atomic (última versión: 20260809000006) con los
-- 3 parámetros nuevos de 20260815000003, todos opcionales (mismos defaults
-- que sus columnas: null, null, 'MXN').
--
-- Cambia el shape de argumentos (17 → 20), así que hace falta DROP explícito
-- del overload de 17 args antes del CREATE OR REPLACE de 20 args — mismo
-- patrón que 20260809000006/000005 (sin el DROP, Postgres deja las DOS
-- funciones conviviendo en el catálogo).
--
-- Idempotente: drop function if exists (overload viejo) + create or replace.
-- Rollback: supabase/migrations/rollbacks/20260815000004_publish_property_atomic_wizard_fields.sql
-- Tests: supabase/tests/06_publish_property_rpc_test.sql (asserts 39-40).

-- Overload viejo (17 args, sin los 3 campos nuevos) — fuera del catálogo primero.

-- Migración 20260908100001 — public.crm_agency_overview: «Sin gestor» incluye al miembro
-- RETIRADO (tarea #278 hardening(269.1); decisión de Abraham 2026-09-07: retirado = suspendido
-- para la banda). create or replace de la RPC de 20260906400001: misma firma, misma forma —
-- cambia solo el CONTENIDO de kind='unmanaged' (aparecen los leads abiertos de ex-miembros que
-- antes nadie señalaba como huérfanos). Aditiva; ninguna tabla tocada. §0.5.2: contrato
-- publicado compatible en forma; el cambio de contenido se anuncia, no requiere OTA (el
-- cliente ya pinta kind=unmanaged + ASIGNAR). Impacto-prod: sin riesgo (RPC de lectura).
-- Rollback: supabase/migrations/rollbacks/20260908100001_crm_agency_overview_removed.sql
-- (la definición íntegra de 20260906400001).
-- Tests: supabase/tests/107_crm_agency_overview_test.sql (plan 77; sección 14
-- SIN_GESTOR_RETIRADO y D-UNMANAGED-REMOVED en la cabecera).
--
-- Único cambio respecto a 20260906400001: el CTE unmanaged_rows — el resto se copia
-- íntegro porque create or replace exige el cuerpo completo.
--
-- ════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  unmanaged_rows as (
    ...
    -- (b) el agente es, HOY, un agency_member status='suspended' O 'removed' (#278) EN ESA
    -- MISMA p_agency_id, sin ninguna fila 'active' ahí.
    -- La RPC es SECURITY DEFINER y salta RLS: debe replicar la MISMA frontera que
    ...
    where l.deleted_at is null
      ...
      -- #278 D-UNMANAGED-REMOVED (decisión de Abraham 2026-09-07: retirado = suspendido para
      -- la banda): el agente cuenta como "sin gestor" si HOY tiene membresía suspended O
      -- removed en p_agency_id y NINGUNA active en p_agency_id. Semi-join, NO join:
      -- agency_members_agency_user_active solo es única para 'active' y redeem-invitation
      -- INSERTA una fila nueva al readmitir (20260905200003) — con join, un retirado
      -- readmitido (removed + active) saldría sin gestor teniendo gestor, y un retirado dos
      -- veces (2 removed) duplicaría el lead. Solo entran leads captados MIENTRAS era miembro
      -- (leads.agency_id ya fijado): private.set_lead_agency_id sigue dando NULL a los leads
      -- NUEVOS de un ex-miembro (regla PII #203.1). reassign_lead_atomic no valida el status
      -- del agente origen, así que ASIGNAR ya funciona para estos leads.
      and exists (
        select 1 from public.agency_members am
        where am.user_id = l.agent_id
          and am.agency_id = p_agency_id
          and am.status in ('suspended', 'removed')
      )
      and not exists (
        select 1 from public.agency_members am
        where am.user_id = l.agent_id
...
comment on function public.crm_agency_overview(uuid) is
  ...
  'suspendidos O RETIRADOS en ESTA agencia sin membresía activa ahí — #278, semi-join; '
  'mecanismo #203/20260904200001) con temperatura y '

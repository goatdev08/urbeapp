// supabase/functions/mint-ad-urls/index.ts
...
// handler.ts y _shared/clients.ts (make_ad_url_minter); los tests importan
// esos módulos directamente y NO pasan por este archivo. Calco de
// mint-poster-urls/index.ts (subtarea 89.1).
...
//   supabase functions deploy mint-ad-urls --import-map supabase/functions/deno.json --use-api

import type { SupabaseClient } from "@supabase/supabase-js";
import { handler } from "./handler.ts";
import { make_ad_url_minter, service_client } from "../_shared/clients.ts";
...
// mismo default que mint-poster-urls/index.ts.
const DEFAULT_TTL_SECONDS = 14400;

/**
 * Arma el HlsSignerConfig leyendo los secrets STREAM_* del entorno y el TTL de
 * app_config. undefined si faltan los secrets → make_ad_url_minter excluye
 * (fail-closed) todas las filas, devolviendo un batch degradado. Espeja
 * build_config de mint-poster-urls/index.ts.
 */
async function build_config(client: SupabaseClient): Promise<HlsSignerConfig | undefined> {
  ...
  try {
    ...
    // ponytail: fallback al default si app_config no responde (mismo criterio que mint-poster-urls)
  }
...
Deno.serve(async (req: Request) => {
  ...
  // usuario autenticado puede pedir URLs; la autorización real es por-item
  // (miembro-o-ad-activo), resuelta dentro de make_ad_url_minter.
  ...
  const adUrlMinter = make_ad_url_minter(client, hls_config);

  return handler(req, { callerVerifier, adUrlMinter });
});

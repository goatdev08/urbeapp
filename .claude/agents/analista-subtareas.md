...
3d. **UI fuera del mockup** (CLAUDE.md §8): si el footprint toca una pantalla, ábrela en `urbea-identidad-visual.html` y compara. Si la subtarea necesita UI o una interacción que el mockup **no dibuja**, márcalo `UI_FUERA_DEL_MOCKUP: {qué falta} · {por qué} · {costo XS|S|M}` para que `/tm-plan` lo pregunte **antes de abrir la rama**. Tú solo lo **señalas**; decide Abraham. Cupo: máx 2 propuestas por tarea (compartido con `REUSO_CON_RESERVA`).

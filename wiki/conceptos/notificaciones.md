...
- `admin_comment_report` (**#289**, 2026-09-11) → `/admin/reports`: al abrir un ciclo de reportes `new` sobre un comentario; dedupe **en el cuerpo del trigger** (no índice) para reabrirse tras `resolve_comment_reports_atomic`; nunca al actor; admins vivos.
...
- `comment_on_my_property` (**#289**) → publicador, `/property/[id]`; insert `visible` o liberación `held_for_review→visible`; nunca si el autor es el publicador; **un aviso por comentario en toda su vida** (índice parcial `(user_id, related_entity_id, type)`).
- `comment_hidden` (**#289**) → autor, `/property/[id]`; por transición a `hidden` (gestor o auto-hide 3/24h — indistinguibles: `data.reason='oculto_por_moderacion'`); cada ciclo avisa. Nada para `held_for_review`. Ver [[comentarios-moderacion]].
- `new_follower` (**#78**, 2026-09-14) → el seguido, `/profile/<seguidor>` (`related_entity_type 'user'`, `related_entity_id` = seguidor, `data.follower_user_id`); body «<nombre público | Alguien> empezó a seguirte.» (nombre de `user_preferences`, nunca el email). **Sin dedupe** por decisión de Abraham: cada follow nuevo avisa, un refollow tras unfollow es otro evento (techo: farmear el aviso; índice parcial si aparece abuso). Ver [[perfil-agente]].
...
- ⭐ **El motivo del rechazo va en el `body`, no solo en `data` (#234 → #237).** `NotificationCard` pinta exclusivamente `title` y `body`, así que un motivo que solo vive en `data.rejection_reason` es un motivo que nadie lee — se descubrió rechazando de verdad en producción, no leyendo el código. Forma única en las cuatro funciones: `v_reason := case when <motivo> ~ '\S' then <motivo> end` y `body || coalesce(' Motivo: ' || v_reason, '')`. Sin motivo el body queda byte por byte como antes. `needs_changes` también lo lleva: «necesita cambios» sin decir cuáles es la versión más inútil del bug, no la más leve.
- ⭐ **«El dato llega» y «la persona lo lee» son dos afirmaciones distintas (#240).** #237 metió el motivo en el `body` con la suite en verde y en el dispositivo seguía sin leerse: iba al final de una frase que abre con la dirección o el título del anuncio, y `NotificationCard` clampeaba el cuerpo a dos líneas. Ahora el motivo tiene **bloque propio** en la tarjeta —etiqueta «Motivo», regla izquierda en arcilla, fondo `accent_tint`— y se lee de `data.rejection_reason`, no del texto del body: es el dato estructurado, y de paso las notificaciones anteriores a #237 también ganan el bloque. El sufijo que la base pega al body se retira en el cliente comparando contra la forma exacta (`' Motivo: ' + motivo`), sin parsear. **Arcilla y no `danger`**: la mala noticia ya la dio el título; el motivo es la parte accionable. Ningún test de base habría cazado esto — salió de tocar la app.
- ⭐ **El guard normaliza lo que se COMUNICA, nunca lo que se PERSISTE.** `ads.rejection_reason`, `property_revisions.rejection_reason` y `admin_actions.reason` guardan el texto crudo que escribió el admin: son registro de lo ocurrido, no mensajes, y normalizarlos sería reescribir la evidencia. Anclado por PR7/AA5 de la suite 93 precisamente porque es contraintuitivo — el guardián avisó que sin ese ancla el próximo refactor «arregla» la asimetría y borra auditoría con la suite en verde.

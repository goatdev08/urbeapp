...
dominio: producto
estado: vivo
fuentes: [.taskmaster/docs/exploraciones/048-follow-comentarios-reportes.md, .taskmaster (tarea #289, derivadas #290 #291 #292), mobile/design-previews/289-comentarios.html]
codigo:
...
- **Gestor** = `private.is_property_comment_manager(property_id)` con la expresión **viva** de `properties_update` tras #202: dueño con membresía vigente en su agencia ∨ `agency_role_of ∈ (owner, admin)` ∨ `is_admin()`. Un dueño **suspendido** pierde el poder de gestor sobre los comentarios de su propia propiedad; owner/admin de agencia lo conservan. Desde **#292** (`20260913100001_can_manage_property_alineado.sql`) `private.can_manage_property()` tiene esta MISMA expresión e `is_property_comment_manager` **delega** en ella (una sola verdad; pgTAP `116` = matriz por rol + candado de paridad + efecto en `videos_select`/`videos_update`, el único consumidor vivo aparte de comments).
...
`useComments` keyset `(created_at desc, id desc)` con `LIMIT+1` para `has_more`, `.eq('property_id')` + `.neq('status','deleted')` explícitos (la RLS decide el resto), identidad vía `agent_public_profiles` (`full_name`/`profile_photo_url`; null → «Usuario eliminado»); `usePostComment` **sin optimismo** (el status lo decide el servidor; el llamador hace `prepend`); `useHideComment.set_status` cubre ocultar/restaurar/borrado propio; `useReportComment` calco de `useReportProperty` sin guard de autor en cliente (lo dice el 42501). Hoja = dirección **A** del preview (compacta, 60 %, input anclado, `Modal` + `KeyboardAvoidingView` + safe-area-context; **#295, 2026-09-14**: el KAV lleva `behavior="padding"` en AMBAS plataformas — con `undefined` en Android no hacía nada y un Modal `statusBarTranslucent` no se redimensiona con `adjustResize`, el teclado tapaba la hoja entera —, `flex: 1` para que su alto sea definido — con alto auto Yoga resolvía el `height: '60%'` de la hoja DOS veces y salía al 36 % de la pantalla, y con teclado al 36 % de lo que quedaba — y `keyboardVerticalOffset={-insets.bottom}` para que el relleno de la barra de navegación quede detrás del teclado); entrada = 4.º botón de `ActionButtons` con `comment_count`. Rail del feed = 289.10. Tras **Ocultar**, toast «Comentario ocultado» con **«Deshacer»** (#291, `producto(289.1)`): `hidden_toast_id` + `setTimeout(HIDDEN_TOAST_MS=4000)`, Deshacer = `set_status(id,'visible')` por el mismo hook; solo tras Ocultar, no tras Restaurar/Eliminar; View absoluto al pie de la lista con tokens ink/paper (patrón `AssignLeadSheet`), sin librería.

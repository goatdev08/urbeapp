...
- **Zona "buscar en esta zona" (#56, vivo)**: `area: { center:{lat,lng}, radius_m } | null` en `FilterState` — segundo eje geoespacial, INDEPENDIENTE de `radius_m`. Es un **override**: con `area != null` el feed y el mapa llaman la RPC `properties_within_radius` con el centro/radio de la zona (NO las coords GPS, NO `UNLIMITED_RADIUS_M`) y **sin expansión ×2** (para que el empty state pueda aparecer); zona **gana** sobre `radius_m` (numérico o null). Con `area == null` el comportamiento de #42/#58/#62 queda **bit-idéntico** (integración puramente aditiva, verificada por guardian). 🔒 Igual que `radius_m`: `area` **NUNCA** pasa por `build_filter_query` (invariante A1, EC-Z3/EC-ZM2) y **NO** cuenta en `get_active_filter_count` (es alcance, no filtro de contenido). **Efímera**: excluida de la persistencia — `save_filters` destructura `area` fuera y `load_filters` fuerza `area:null` tras el merge (defensivo contra JSON legacy; EC-AREA-1..4). Combina con los demás filtros (zona **Y** precio/tipo); `clear_filters` **preserva** `area` (ejes independientes). Fórmula viewport→área en `map/lib/viewportToArea.ts`. Ver [[mapa-y-ubicacion]].

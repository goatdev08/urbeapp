...
- Paginación del feed (#42): **offset sobre los ids de la RPC de proximidad** (ya NO cursor `created_at`); el `nextCursor` se calcula sobre los ids → filtros post-slice pueden dar páginas cortas (aceptado en demo, `// ponytail:`). Con `radius_m=null` (#58) el offset va directo en PostgREST vía `.range(offset, offset+9)` (página llena → nextCursor=offset+10; vacía/corta → null).

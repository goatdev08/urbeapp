...
- Owner-view de la agencia (#28, vivo): `hooks/useAgencyRole.ts` (**CRÍTICA**, 6 tests — detecta owner por `agency_members`), `hooks/useAgencyAgents.ts` (**CRÍTICA**, 8 tests — lista de agentes vía embed `agency_members→users(first_name,last_name,avatar_url)`; ⚠️ nombre/foto de `users`, NO `user_preferences` (el owner no puede leer el user_preferences ajeno por RLS); solo ejecuta si owner), `hooks/useAgentLeads.ts` extendido a `(agentId?)` (**15 tests**; `.eq('agent_id',id)` condicional; embed del buscador con FK explícita `users!leads_user_id_fkey` — fix del "Could not embed" por doble FK leads→users), `components/AgentSelector.tsx` (rail de chips, no crítico). Todo en `CRMScreen.tsx`. Verificado en vivo en el emulador (selector con nombres reales + filtrado por agente). Suite leads: 42/42, tsc 0. ⚠️ El nombre del BUSCADOR del lead sigue saliendo null ("Usuario sin nombre") por RLS `users_select`/`user_prefs_select` (el agente no puede leer la identidad del buscador) → **tarea #30** (política RLS).

...
codigo: [mobile/src/theme/theme.ts, mobile/src/components/RailIcon.tsx, mobile/src/components/FollowButton.tsx, mobile/app/_layout.tsx, mobile/src/components/PropertyGridCard.tsx, mobile/src/components/PrimaryButton.tsx, mobile/src/components/IsotipoMark.tsx, mobile/src/components/UrbeaLockup.tsx, mobile/app/login.tsx, mobile/app/(protected)/(tabs)/_layout.tsx]
...
**Cómo usarlo para acotar alcance (scope) por tarea:** cada `<div class="phone-fig">` numerado es **una pantalla = el techo de alcance** de su tarea. El `phone-cap` dice qué incluye. No **implementes** UI que el mockup no muestra (evita scope-creep) — pero **tampoco la recortes en silencio**: si una pantalla real necesita algo ausente del mockup se **propone** con las 2 opciones (en conjunto vs derivada `producto(<origen>)`, default la derivada). La regla operativa completa vive en **CLAUDE.md §8** (*Techo CON PROPUESTA*); aquí no se duplica.
...
- **Iconografía Phosphor bold** (#43.1): TODA la app migró de `@expo/vector-icons` (Ionicons) a **`phosphor-react-native`** (variante bold, `weight="fill"` en estados activos) en 15 archivos + tab bar (HouseLine/Bookmarks/MapPinSimple/Ranking/UserCircle). `@expo/vector-icons` removido.
- **Pin único = `MapPinSimple`** (#185, 2026-08-16): un solo ícono de pin en TODA la app — los 3 mapas (vía `MapPinIcon`), la dirección del detalle, el chip de zona, las sugerencias de colonia, el estado vacío del feed y el tab de Mapa en ambas plataformas. En Android el tab dibuja el componente Phosphor; en iOS `NativeTabs` no acepta componentes React y consume PNG (`assets/tab-icons/map-pin-simple-{regular,fill}[@2x,@3x].png`) — el salvia del seleccionado lo pone `tintColor` + `renderingMode="template"`, nunca el PNG. ⚠️ **Gotcha al rasterizar** (ver abajo, `qlmanage`): un PNG opaco se pinta como un CUADRADO sólido de color en modo template.
  - **En los MAPAS el pin va a dos tonos** (#186): `weight="duotone"` + `duotoneOpacity={1}` (el default 0.2 lava el relleno) → contorno con `color` y disco con `duotoneColor`. Contorno = tinta en tema claro / marfil en oscuro vía `useColorScheme` (**primera detección de tema del dispositivo en la app**; `userInterfaceStyle: 'automatic'` ya estaba en `app.config.js`), limitado a iOS porque los tiles de Google en Android no oscurecen con el sistema. Disco = color de marca, o el código por operación en el mapa global (renta salvia / venta arcilla — se conservó a propósito: es la única señal que distingue renta de venta de un vistazo). En el TAB sigue siendo silueta (`fill`/`regular`), no duotone.
  - ⭐ **#294 (2026-09-14): en los MARCADORES el pin es una chincheta vectorial** — réplica del 📍 de iOS dibujada con `react-native-svg` dentro de `MapPinIcon.tsx` (cabeza esférica con gradiente radial + brillo, cuello y aguja con gradiente lineal, sombra elipse; sin `filter`). Motivo: el emoji como texto lo pinta la fuente de cada sistema (Apple Color Emoji vs Noto) y la de Apple no se puede empaquetar; con el vector Android e iOS dibujan lo mismo. `color` tiñe la CABEZA (renta salvia / venta arcilla se conserva), vertical sin inclinación, colores de Urbea y no el rojo (decisión de Abraham sobre preview HTML). Lienzo cuadrado con la punta abajo-centro → los tres consumidores no cambian. El guard de tema de #186 se retiró (ya no hay contorno). Los pines INLINE y el tab siguen con Phosphor `MapPinSimple`.
- **Icono de app** (#43.3): `assets/icon.png` (iOS, verde full-bleed + símbolo carnita) + adaptive Android (`android-icon-foreground/background.png`, sin monochrome), `app.config.js` backgroundColor `#1A5E44`. Assets rasterizados de SVG con **`qlmanage`** (Quick Look de macOS) + `sips` — resuelve el bloqueo SVG→PNG histórico (#32) sin instalar convertidor. ⚠️ **`qlmanage` puede aplanar el SVG sobre BLANCO** (alpha=255 en todo el lienzo) y `sips -g hasAlpha` responde `yes` igual (hay canal alpha, todo opaco) — mentira útil que costó un ciclo en #185: el tab bar de iOS pintó un cuadrado verde sólido en vez del ícono. Receta segura para íconos de tab (negro sobre blanco): tras rasterizar, **reconstruir el alpha desde la luminancia** (blanco→0, negro→255, grises→antialias) y dejar el RGB en negro; el `renderingMode="template"` hace el resto. Verificar SIEMPRE decodificando el PNG (no basta `hasAlpha`).
...
- **Rail del feed y del detalle — outline sin cápsula (#293, 2026-09-13, doc 049 dirección B):** los botones de acción son ícono Phosphor `bold` 28 px blanco flotando sobre el video, sin círculo ni fondo; el contraste lo da **`RailIcon.tsx`** (copia negra rgba(0,0,0,.4) desplazada 1 px detrás del ícono — `textShadow` no aplica a SVG y `elevation` no dibuja sobre transparente) y los conteos (like/comentarios, `format_count`, ocultos en 0) llevan `textShadow` rgba(0,0,0,.85). Única nota de color: el logo de WhatsApp **relleno** en verde `#25D366`, sin círculo (polish de Abraham; antes era un círculo sólido). Paso uniforme del rail: caja 46 + gap 10, ranura de conteo siempre presente (`marginTop 8`, centrada entre glifos), base del rail = base del bloque de info. Fila del agente: avatar 36 · nombre · píldora `FollowButton` **pegada al nombre** (gap 8), píldora radio 8 / borde 1 px en ambas variantes. Preview aprobado por Abraham (frame claro + oscuro, variantes A/B/C): `.taskmaster/docs/exploraciones/049-rediseno-layout-overlay-feed/preview/overlay.html`; mockup canónico pantalla 4 sincronizado. El `back_btn` del detalle sigue siendo píldora glass (no es rail).
- **Chrome superior del feed — 5 tabs deslizables + filtros a la izquierda (#296, 2026-09-14, doc 050 variante B):** fila `Para ti · Siguiendo · Nuevos · Venta · Renta` en ScrollView horizontal que arranca tras el botón de filtros (`left: s_16 + 40 + s_8`) y llega al borde derecho (sin hueco reservado; `paddingRight s_24`, sin fade por falta de máscara nativa); activa = pill `colors.primary` con texto blanco `sans_bold`, inactivas blanco 72 % `sans_semibold` + sombra de texto (se descartó el subrayado: ya falló en #241). Botón de filtros a la **izquierda** (`left: s_16`, 40 px, ink_feed 60 %). Badge legal «Patrocinado» a la **derecha una fila abajo**, en la banda de `RefreshingChip`/`ZoneActiveChip` (decisión de Abraham al aprobar: la fila completa para las tabs pesa más que el hueco fijo). Preview aprobado: `.taskmaster/docs/exploraciones/050-tabs-superiores-feed-y-filtro-izquierda/preview/chrome.html`; mockup canónico pantalla 4 sincronizado.

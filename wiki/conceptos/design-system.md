...
**Cómo usarlo para acotar alcance (scope) por tarea:** cada `<div class="phone-fig">` numerado es **una pantalla = el techo de alcance** de su tarea. El `phone-cap` dice qué incluye. No agregues UI que el mockup no muestra (evita scope-creep); si una pantalla real necesita algo ausente del mockup, es **trabajo nuevo** → se escribe directo en `tasks.json` (`add-task`/`expand` están rotos, CLAUDE.md §4), no se cuela en la tarea en curso.

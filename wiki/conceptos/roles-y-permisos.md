...
## Reglas / gotchas
- ⭐ **Nunca degradar a un admin al promoverlo a agente** (#225, 2026-08-31). Toda ruta que suba a alguien a `agent` debe escribir `set role = case when role = 'admin' then role else 'agent' end`. Ya lo hacían los DOS triggers de aprobación desde `20260805000010`; `admin_create_agency_atomic` se quedó fuera de ese barrido y degradaba a un administrador asignado owner de una organización nueva —quitándole el panel— hasta `20260831000001`. 🔒 **El guard va en el VALOR, nunca en el WHERE**: un `where … and role <> 'admin'` salva el rol pero se salta también la denormalización de `agency_id`, y sin ella la RLS `properties_insert` (que exige `agency_role_of(agency_id) is not null`) le impide publicar. Anclado por los asserts 22–24 de `supabase/tests/05_admin_create_agency_test.sql`, con el mutante del "fix perezoso" verificado.
